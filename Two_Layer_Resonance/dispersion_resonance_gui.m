function dispersion_resonance_gui
    ...
    cfg.ymin_init   = 0.0;    % Y-Axis Lower Limit (Set to 0 as requested)
    ...
    function updatePlot(~, ~)
        ...
        % 2. Find and Plot Intersections (Yellow with Black Perimeter)
        k_int_all = [];
        om_int_all = [];
        ...
        for i = 1:4
            ...
            for j = 1:4
                ...
                for k_idx = 1:length(idx_list)
                    ...
                    k_val = x_a + frac*(x_b - x_a);
                    
                    val_a = y1(id); val_b = y1(id+1);
                    om_val = val_a + frac*(val_b - val_a);
                    
                    k_int_all(end+1) = k_val; %#ok<AGROW>
                    om_int_all(end+1) = om_val; %#ok<AGROW>
                end
            end
        end
        
        if ~isempty(k_int_all)
             % Calculate distances to Origin and Shift Point to filter them out
             dist_origin = sqrt(k_int_all.^2 + om_int_all.^2);
             dist_shift  = sqrt((k_int_all - k0_val).^2 + (om_int_all - Omega0).^2);
             tol = 1e-3;
             
             % Mask: Keep only points that are NOT origin, NOT shift point, and inside Y-limits
             mask = (dist_origin > tol) & ...
                    (dist_shift > tol) & ...
                    (om_int_all >= ymin_val) & (om_int_all <= ymax_val);
                    
             plot(ax, k_int_all(mask), om_int_all(mask), 'o', ...
                 'Color', 'k', ...             % Black Perimeter
                 'MarkerFaceColor', [1 1 0], ... % Bright Yellow Fill
                 'MarkerSize', 9, ...         % 30% Bigger
                 'LineWidth', 1);
        end

        % 3. Draw Special Points on Top (Black Filled)
        % Origin
        plot(ax, 0, 0, 'ko', 'MarkerFaceColor', 'k', 'MarkerSize', 9);
        % Shift Point
        plot(ax, k0_val, Omega0, 'ko', 'MarkerFaceColor', 'k', 'MarkerSize', 9);
